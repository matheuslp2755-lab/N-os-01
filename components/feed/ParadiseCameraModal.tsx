...
const PREMIUM_PACK: Record<string, EffectConfig> = {
    beauty_glow: { id: 'beauty_glow', name: 'Neos Beauty Glow', label: '✨', exposure: 1.15, contrast: 1.05, saturation: 1.1, vibrance: 1.2, temp: 8, magenta: 2, sharpness: 1.0, grain: 0, skinSoft: 0.9, glow: 0.5, vignette: 0.05 },
    sharp_boy: { id: 'sharp_boy', name: 'Neos Sharp Boy', label: '👔', exposure: 1.05, contrast: 1.2, saturation: 1.0, vibrance: 1.0, temp: -5, magenta: 0, sharpness: 2.5, grain: 5, skinSoft: 0.2, glow: 0.1, vignette: 0.12 },
    sunset: { id: 'sunset', name: 'Neos Sunset Paradise', label: '🌅', exposure: 1.1, contrast: 1.1, saturation: 1.3, vibrance: 1.4, temp: 25, magenta: 5, sharpness: 1.4, grain: 0, skinSoft: 0.4, glow: 0.4, vignette: 0.1 },
    night: { id: 'night', name: 'Neos Night Shine', label: '🌃', exposure: 1.4, contrast: 1.25, saturation: 1.1, vibrance: 1.1, temp: 5, magenta: -2, sharpness: 1.6, grain: 12, skinSoft: 0.3, glow: 0.6, vignette: 0.2 },
    golden: { id: 'golden', name: 'Neos Golden Hour Pro', label: '🌞', exposure: 1.1, contrast: 1.05, saturation: 1.15, vibrance: 1.2, temp: 18, magenta: 3, sharpness: 1.2, grain: 0, skinSoft: 0.7, glow: 0.3, vignette: 0.08 },
    cinematic: { id: 'cinematic', name: 'Neos Cinematic Vibe', label: '🎬', exposure: 0.9, contrast: 1.3, saturation: 0.8, vibrance: 0.9, temp: -12, magenta: -4, sharpness: 1.8, grain: 20, skinSoft: 0, glow: 0.4, vignette: 0.3 },
    pop: { id: 'pop', name: 'Neos Asterix Pop', label: '💥', exposure: 1.0, contrast: 1.2, saturation: 1.4, vibrance: 1.5, temp: 10, magenta: 0, sharpness: 1.5, grain: 0, skinSoft: 0.2, glow: 0.2, vignette: 0.05 },
    elegant: { id: 'elegant', name: 'Neos Elegant Pro', label: '💎', exposure: 1.0, contrast: 1.15, saturation: 0.85, vibrance: 0.9, temp: -10, magenta: 2, sharpness: 2.0, grain: 5, skinSoft: 0.4, glow: 0.1, vignette: 0.15 },
    party: { id: 'party', name: 'Neos Vibrant Party', label: '🎉', exposure: 1.25, contrast: 1.2, saturation: 1.3, vibrance: 1.3, temp: 5, magenta: 4, sharpness: 1.2, grain: 8, skinSoft: 0.1, glow: 0.7, vignette: 0.1 },
    mood: { id: 'mood', name: 'Neos Mood Blue', label: '❄️', exposure: 1.05, contrast: 1.1, saturation: 0.7, vibrance: 0.8, temp: -30, magenta: -8, sharpness: 1.4, grain: 15, skinSoft: 0, glow: 0.3, vignette: 0.25 }
};
...
const ParadiseCameraModal: React.FC<ParadiseCameraModalProps> = ({ isOpen, onClose }) => {
    ...
    // Fator de zoom: MM maior = Fator de escala maior
    ...
    const applyQualityPipeline = (ctx: CanvasRenderingContext2D, w: number, h: number, config: EffectConfig, isFinal: boolean) => {
        ...
        // 2. Glow (Highlights difusos)
        if (config.glow && config.glow > 0) {
            ctx.globalCompositeOperation = 'screen';
            ctx.globalAlpha = config.glow * 0.35;
            ctx.filter = `blur(${Math.round(w * 0.025)}px) brightness(1.4)`;
            ctx.drawImage(ctx.canvas, 0, 0);
            ctx.globalCompositeOperation = 'source-over';
            ctx.globalAlpha = 1.0;
        }

        // 3. Suavização (Soft Skin)
        if (config.skinSoft && config.skinSoft > 0) {
            ctx.globalAlpha = config.skinSoft * 0.15;
        ...
        if (config.grain && config.grain > 0) {
            ...
            ctx.globalAlpha = config.grain / 250;
            for(let i=0; i<150; i++){
        ...
        // 6. Selos Profissionais Nelcel (Somente Captura)
        if (isFinal) {
            ctx.filter = 'none';
            const now = new Date();
            // Formato 'YY MM DD
            ...
            ctx.fillStyle = '#fde047'; // Amarelo Dazz
            ctx.shadowColor = 'rgba(0,0,0,0.6)';
            ctx.shadowBlur = 5;
            ctx.fillText(dateStr, w * 0.07, h * 0.93);
            ...
            ctx.fillText("NELCEL", w * 0.93, h * 0.93);
        }
    ...
    const executeCapture = () => {
        ...
        // Recorte preciso: apenas o que está dentro do "quadro" (frame guide)
        const cropW = vw / zoom;
        const cropH = vh / zoom;
        const sx = (vw - cropW) / 2;
        const sy = (vh - cropH) / 2;
    ...
    const saveCustomFilter = () => {
        const name = prompt("Nome do seu efeito Nelcel:", "Lente Custom");
        if (!name) return;
    ...
    return (
        <div className="fixed inset-0 bg-black flex flex-col overflow-hidden touch-none h-[100dvh] text-white font-sans z-[600]">
            ...
            <div className="flex-grow relative bg-zinc-950 flex items-center justify-center overflow-hidden">
                ...
                {/* Guia de Frame (Recorte) */}
                <div className="absolute inset-0 pointer-events-none flex items-center justify-center">
                    <div 
                        className="border-2 border-white/30 rounded-[2rem] shadow-[0_0_0_2000px_rgba(0,0,0,0.5)] transition-all duration-500 ease-out"
                        style={{ width: `${100/zoom}%`, aspectRatio: '3/4' }}
                    >
                        <div className="absolute top-4 right-4 flex flex-col items-end opacity-40">
                             <span className="text-[10px] font-black">OPTICAL RE-ZOOM</span>
                             <div className="h-px w-8 bg-white mt-1"></div>
                        </div>
                        <div className="absolute bottom-4 left-4 opacity-40">
                            <span className="text-[10px] font-black">{lensMM}MM f/2.8</span>
                        </div>
                    </div>
                </div>
            </div>

            <footer className="bg-black px-4 pb-12 pt-4 border-t border-white/5 z-50 shadow-[0_-20px_40px_rgba(0,0,0,0.8)]">
                {isCreatingFilter ? (
                    <div className="flex flex-col gap-6 animate-slide-up">
                        {activeEditTool ? (
                            <div className="px-6 py-5 bg-zinc-900 rounded-[2.5rem] space-y-5 shadow-2xl border border-white/5">
                                <div className="flex justify-between items-center">
                                    <span className="text-[10px] font-black uppercase tracking-widest text-sky-400">
                                        {editTools.find(t => t.id === activeEditTool)?.name}
                                    </span>
                                    <button onClick={() => setActiveEditTool(null)} className="text-xs font-black uppercase tracking-widest text-white">OK</button>
                                </div>
                                <input 
                                    type="range" 
                            ...
                            <div className="flex gap-4 overflow-x-auto no-scrollbar py-2 px-2">
                                {editTools.map(tool => (
                                    ...
                                    >
                                        <div className="w-14 h-14 rounded-full bg-zinc-900 border border-white/10 flex items-center justify-center text-xl hover:bg-zinc-800 transition-all shadow-xl">
                                            {tool.icon}
                                        </div>
                        ...
                        <div className="flex gap-4 px-4">
                            <button onClick={() => setIsCreatingFilter(false)} className="flex-1 py-4 bg-zinc-900 rounded-2xl font-black text-[10px] uppercase tracking-widest border border-white/5">Cancelar</button>
                            <button onClick={saveCustomFilter} className="flex-1 py-4 bg-sky-500 text-white rounded-2xl font-black text-[10px] uppercase tracking-widest shadow-xl shadow-sky-500/20">Salvar Lente</button>
                        </div>
                    </div>
                ) : (
                    <div className="flex flex-col gap-6">
                        <div className="flex gap-4 overflow-x-auto no-scrollbar py-2 px-2 items-end">
                            <button onClick={() => setIsCreatingFilter(true)} className="flex flex-col items-center shrink-0 mb-1">
                                <div className="w-12 h-12 rounded-full bg-zinc-900 border-2 border-dashed border-white/20 flex items-center justify-center text-xl text-zinc-500 hover:border-white transition-all">+</div>
                            ...
                            {Object.values(allPresets).map(eff => (
                                <button key={eff.id} onClick={() => setActiveVibe(eff.id)} className={`flex flex-col items-center shrink-0 transition-all ${activeVibe === eff.id ? 'scale-110 opacity-100' : 'opacity-30'}`}>
                                    <div className={`w-14 h-14 rounded-full flex items-center justify-center text-xl border ${activeVibe === eff.id ? 'bg-white text-black border-white shadow-[0_0_25px_rgba(255,255,255,0.4)]' : 'bg-zinc-900 border-white/10 text-zinc-500'}`}>{eff.label}</div>
                                    <span className="text-[8px] font-black uppercase mt-2 tracking-widest text-center whitespace-nowrap">{eff.name.split(' ').slice(-1)}</span>
                                </button>
                            ))}
                        </div>
                        <div className="flex items-center justify-between px-10">
                            ...
                            <div className="w-14 h-14 bg-zinc-900/40 rounded-full flex items-center justify-center border border-white/5 opacity-40">
            ...
            {fullscreenImage !== null && (
                <div className="fixed inset-0 z-[800] bg-black flex flex-col animate-fade-in">
                    ...
                    <div className="flex-grow flex items-center justify-center p-4">
                        <img src={capturedImages[fullscreenImage]} className="max-h-full max-w-full object-contain rounded-[2rem] shadow-[0_0_80px_rgba(0,0,0,0.6)]" alt="full" />
                    </div>
