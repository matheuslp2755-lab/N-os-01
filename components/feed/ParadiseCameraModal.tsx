...
const ParadiseCameraModal: React.FC<ParadiseCameraModalProps> = ({ isOpen, onClose }) => {
    ...
    const executeCapture = () => {
        ...
        if (ctx) {
            ...
            // Data Vertical Amarela (Lado Esquerdo, em pé)
            const now = new Date();
            const dateStr = `${now.getDate().toString().padStart(2, '0')} ${(now.getMonth() + 1).toString().padStart(2, '0')} ${now.getFullYear().toString().slice(-2)}`;
            ctx.save();
            ctx.translate(canvas.width * 0.05, canvas.height * 0.85);
            ctx.rotate(-Math.PI / 2); // Rotaciona para ficar em pé (reta)
            ctx.font = `bold ${Math.floor(canvas.width * 0.035)}px Courier, monospace`;
            ctx.fillStyle = '#fbbf24'; // Amarelo vibrante
    ...
    return (
        <div className="fixed inset-0 bg-black z-[600] flex flex-col overflow-hidden text-white font-sans touch-none select-none">
            ...
            {/* Tela de Visualização de Mídia Capturada */}
            {viewingMedia && (
                <div className="fixed inset-0 z-[800] bg-black flex flex-col animate-fade-in">
                    <header className="p-6 flex justify-between items-center bg-black/40 backdrop-blur-xl border-b border-white/10 z-10">
                        ...
                        <span className="text-[10px] font-black uppercase tracking-widest">Visualização Paradise</span>
                        <button onClick={() => saveMedia(viewingMedia)} className="p-2 bg-sky-500 rounded-full shadow-lg shadow-sky-500/20">
                    ...
                    <div className="flex-grow flex items-center justify-center p-4">
                        <img src={viewingMedia} className="max-w-full max-h-full rounded-3xl shadow-2xl object-contain" alt="Captured" />
                    </div>
                </div>
            )}
            ...
            <footer className="bg-black pt-4 pb-12 px-8">
                ...
                <div className="flex items-center justify-between">
                    <button 
                        onClick={() => capturedMedia[0] && setViewingMedia(capturedMedia[0])}
                        className="w-14 h-14 rounded-2xl bg-zinc-900 border border-white/10 overflow-hidden shadow-2xl active:scale-95 transition-all cursor-pointer"
                    >
