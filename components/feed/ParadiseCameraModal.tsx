...
type VibeEffect = 
    ...
    | 'flash_raw'
    | 'club_cinema'
    | string; // Permite IDs de filtros customizados
...
const ParadiseCameraModal: React.FC<ParadiseCameraModalProps> = ({ isOpen, onClose }) => {
    ...
    const [newFilter, setNewFilter] = useState<EffectConfig>({
        id: '', name: '', label: '⭐', exposure: 1.0, contrast: 1.0, saturation: 1.0, temp: 0, sharpness: 1.0, grain: 0, vignette: 0, isCustom: true
    });
    ...
    const saveCustomFilter = () => {
        if (!newFilter.name.trim()) return alert("Dê um nome ao seu efeito!");
        const filterToSave = { ...newFilter, id: `custom_${Date.now()}` };
    ...
    const handleDeleteFilter = (id: string, e: React.MouseEvent) => {
        e.stopPropagation();
        if (window.confirm("Remover este efeito permanente?")) {
            const updated = customFilters.filter(f => f.id !== id);
            setCustomFilters(updated);
            localStorage.setItem('neos_custom_filters', JSON.stringify(updated));
            setActiveVibe('party_flash');
        }
    };
    ...
    const handleSaveToPost = async (dataUrl: string) => {
        ...
        try {
            ...
            await addDoc(collection(db, 'posts'), {
                ...
                caption: `🎞️ Paradise: ${activeConfig.name} (${lensMM}mm)`,
                likes: [],
    ...
    return (
        <div className="fixed inset-0 bg-black flex flex-col overflow-hidden touch-none h-[100dvh] text-white font-sans z-[600]">
            ...
            <header className="absolute top-0 left-0 right-0 p-6 flex justify-between items-center z-50">
                <button onClick={onClose} className="w-10 h-10 bg-black/40 backdrop-blur-xl rounded-full flex items-center justify-center border border-white/10 text-xl active:scale-90">&times;</button>
                ...
                <button onClick={() => setFacingMode(prev => prev === 'user' ? 'environment' : 'user')} className="w-10 h-10 bg-black/40 backdrop-blur-xl rounded-full flex items-center justify-center border border-white/10 active:scale-90">
            ...
            <div className="flex-grow relative bg-zinc-950 flex items-center justify-center overflow-hidden">
                ...
                {/* Efeito em criação Overlay */}
                {isCreatingFilter && (
                    <div className="absolute inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-end animate-fade-in">
                        <div className="bg-zinc-900 w-full rounded-t-[3rem] p-8 space-y-8 pb-12 max-h-[70vh] overflow-y-auto no-scrollbar">
                            <div className="flex justify-between items-center">
                                <h3 className="text-xl font-black italic text-sky-400 uppercase tracking-tighter">Novo Efeito Paradise</h3>
                                <button onClick={() => setIsCreatingFilter(false)} className="text-zinc-500 font-bold">X</button>
                            </div>

                            <div className="space-y-6">
                                <div className="space-y-2">
                                    <label className="text-[10px] font-black uppercase text-zinc-500 tracking-widest">Nome do Efeito</label>
                                    <input 
                                        type="text" 
                                        placeholder="Ex: Noite de Verão" 
                                        className="w-full bg-zinc-800 border-none rounded-2xl p-4 text-sm font-bold outline-none"
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                    <div className="space-y-3">
                                        <label className="text-[10px] font-black uppercase text-zinc-500 flex justify-between">Luz <span>{Math.round(newFilter.exposure * 100)}%</span></label>
                                        <input type="range" min="0.5" max="2" step="0.05" value={newFilter.exposure} onChange={e => setNewFilter(prev => ({...prev, exposure: parseFloat(e.target.value)}))} className="w-full h-1.5 bg-zinc-800 rounded-lg appearance-none accent-sky-500" />
                                    </div>
                                    <div className="space-y-3">
                                        <label className="text-[10px] font-black uppercase text-zinc-500 flex justify-between">Contraste <span>{Math.round(newFilter.contrast * 100)}%</span></label>
                                        <input type="range" min="0.5" max="1.5" step="0.05" value={newFilter.contrast} onChange={e => setNewFilter(prev => ({...prev, contrast: parseFloat(e.target.value)}))} className="w-full h-1.5 bg-zinc-800 rounded-lg appearance-none accent-sky-500" />
                                    </div>
                                    <div className="space-y-3">
                                        <label className="text-[10px] font-black uppercase text-zinc-500 flex justify-between">Temperatura <span>{newFilter.temp > 0 ? `+${newFilter.temp}` : newFilter.temp}</span></label>
                                        <input type="range" min="-30" max="30" step="1" value={newFilter.temp} onChange={e => setNewFilter(prev => ({...prev, temp: parseInt(e.target.value)}))} className="w-full h-1.5 bg-zinc-800 rounded-lg appearance-none accent-yellow-500" />
                                    </div>
                                    <div className="space-y-3">
                                        <label className="text-[10px] font-black uppercase text-zinc-500 flex justify-between">Nitidez <span>{Math.round(newFilter.sharpness * 100)}%</span></label>
                                        <input type="range" min="0.5" max="2.5" step="0.1" value={newFilter.sharpness} onChange={e => setNewFilter(prev => ({...prev, sharpness: parseFloat(e.target.value)}))} className="w-full h-1.5 bg-zinc-800 rounded-lg appearance-none accent-sky-500" />
                                    </div>
                                </div>
                            </div>

                            <Button onClick={saveCustomFilter} className="!py-5 !rounded-3xl !font-black !uppercase !tracking-widest !bg-white !text-black shadow-2xl active:scale-95 transition-all">Salvar Efeito</Button>
                        </div>
                    </div>
                )}
            </div>
            ...
            <footer className="bg-black px-4 pb-12 pt-6 border-t border-white/5 z-50">
                <div className="flex flex-col gap-6">
                    <div className="flex gap-4 overflow-x-auto no-scrollbar py-2 px-2">
                        {/* Botão de Criar Filtro */}
                        <button 
                            onClick={() => setIsCreatingFilter(true)}
                            className="flex flex-col items-center shrink-0 group"
                        >
                            <div className="w-14 h-14 rounded-2xl bg-zinc-900 border-2 border-dashed border-white/20 flex items-center justify-center text-xl text-zinc-500 group-hover:border-white group-hover:text-white transition-all">+</div>
                            <span className="text-[8px] font-black uppercase mt-2 tracking-widest text-zinc-500 group-hover:text-white">Criar</span>
                        </button>

                        {/* Presets Nativos e Customizados */}
                        {Object.values(allPresets).map(eff => (
                            <button 
                                key={eff.id} 
                                onClick={() => setActiveVibe(eff.id)}
                                onContextMenu={(e) => eff.isCustom && handleDeleteFilter(eff.id, e)}
                                className={`flex flex-col items-center shrink-0 transition-all relative ${activeVibe === eff.id ? 'scale-110 opacity-100' : 'opacity-30'}`}
                            >
                                ...
                                <span className="text-[8px] font-black uppercase mt-2 tracking-widest max-w-[60px] truncate">{eff.name}</span>
                                {eff.isCustom && (
                                    <div 
                                        className="absolute -top-1 -right-1 w-4 h-4 bg-red-500 rounded-full flex items-center justify-center text-[8px] font-bold"
                                        onClick={(e) => handleDeleteFilter(eff.id, e)}
                                    >
                                        &times;
                                    </div>
                                )}
